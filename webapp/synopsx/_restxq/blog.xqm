...
function home(){
  let $data    := synopsx.models.tei.article:listArticles()
...
function article($entryId as xs:string){
  let $data    := synopsx.models.tei.article:article($entryId)
  let $options := map {}
  let $layout  := $G:TEMPLATES || 'blogHtml5.xhtml'
...
function articleXml($entryId as xs:string){
  let $lang := 'fr'
  let $data := synopsx.models.tei.article:getXmlTeiById($entryId) 
